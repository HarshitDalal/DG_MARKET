    ...
    if not request.user.is_authenticated:
        if request.method == "POST":
            ...
            if regfm.is_valid():
                ...
                messages.error(
                    request, 'Password Could Not Be Same As Email Or Username')
        else:
...
def contact(request):
    if request.method == 'POST':
        ...
        if confm.is_valid():
            ...
            contacts = ContactUs(name=name, email=email,
                                 mobile=mobile, decp=decp)
            send_mail(
    ...
    if request.user.is_superuser == True:
        if request.method == "POST":
            ...
            if fm.is_valid():
                fm.save()
                messages.success(
                    request, f'{request.user} Profile Successfully Update')
        else:
            fm = AdminProfileForm(instance=request.user)
    else:
        if request.method == "POST":
            fm = UserDetailsForm(request.POST, request.FILES,
...
def orderitem(request):
    try:
        if request.method == 'POST':
            ...
            orders = Order(Buyer=buyer, Email=useremail, Address=request.user.userdetails.Address, PinCode=request.user.userdetails.PinCode, Phone=request.user.userdetails.Mobile,
                           ProductImg=img, ProductName=name, ProductQty=qty, ProductSize=size, ProductColor=color, ProductPrice=price, SellerName=seller, SellerEmail=selleremail, SellerPhone=sellerphone,)
            orders.save()
            msg1 = (
                'You Got An Order',
            ...
            messages.success(
                request, 'Order Successfully Added Call Seller For Payment Method')
